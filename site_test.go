...
func TestDB_Equal(t *testing.T) {
	...
	site4 := Site{URL: "http://site1.example/blah", Filter: "foo", ContentType: "text", UseChrome: true, Interval: time.Minute}
	...
	if want, got := false, site1.Equals(&site4); want != got {
		t.Fatalf("Expected site1.Equals(site4) == %v, got %v", want, got)
	}
}
